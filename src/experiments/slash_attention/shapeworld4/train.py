...
def slash_slot_attention(exp_name , exp_dict):
    ...
    # Create RTPT object
    rtpt = RTPT(name_initials=exp_dict['credentials'], experiment_name='SLASH Shapeworld4', max_iterations=int(exp_dict['epochs']))
    ...
    train_dataset_loader = torch.utils.data.DataLoader(SHAPEWORLD4('../../data/shapeworld4/',"train"), shuffle=True,batch_size=exp_dict['bs'],pin_memory=False, num_workers=8)
    test_dataset_loader = torch.utils.data.DataLoader(SHAPEWORLD4('../../data/shapeworld4/',"val", ret_obj_encoding=True), shuffle=False,batch_size=exp_dict['bs'],pin_memory=False, num_workers=8)
    ...
    for e in range(start_e, exp_dict['epochs']):
        ...
        #forward test batch
        inference, ll = SLASHobj.forward_slot_attention_pipeline(slot_net=slot_net, dataset_loader= test_dataset_loader)
