...
def slash_slot_attention(exp_name , exp_dict):
    ...
    writer = SummaryWriter(os.path.join("runs", exp_name), purge_step=0)
    ...
    #metric lists
    test_ll_list = [] #stores log likelihoods for test set with condition b
    ...
    # load datasets
    print("loading data...")
    train_dataset_loader = torch.utils.data.DataLoader(SHAPEWORLD_COGENT('../../data/shapeworld_cogent/',"train_a"), shuffle=True,batch_size=exp_dict['bs'],pin_memory=False, num_workers=8)
    test_dataset_loader_a = torch.utils.data.DataLoader(SHAPEWORLD_COGENT('../../data/shapeworld_cogent/',"val_a", ret_obj_encoding=True), shuffle=False,batch_size=exp_dict['bs'],pin_memory=False, num_workers=8)
    test_dataset_loader_b = torch.utils.data.DataLoader(SHAPEWORLD_COGENT('../../data/shapeworld_cogent/',"val_b", ret_obj_encoding=True), shuffle=False,batch_size=exp_dict['bs'],pin_memory=False, num_workers=8)
    ...
    for e in range(start_e, exp_dict['epochs']):
        ...
        loss = SLASHobj.learn(dataset_loader = train_dataset_loader, slot_net=slot_net, method='slot', p_num = exp_dict['p_num'],
                        epoch=1, batchSize=exp_dict['bs'], train_slot= True)
        ...
        inference, ll = SLASHobj.forward_slot_attention_pipeline(slot_net=slot_net, dataset_loader= test_dataset_loader_a)
        ...
        print("condition b")
         #forward test batch a
        inference, ll = SLASHobj.forward_slot_attention_pipeline(slot_net=slot_net, dataset_loader= test_dataset_loader_b)
