...
class SoftPositionEmbed(nn.Module):
    ...
    def forward(self, inputs):
        #print("positional embedding",inputs.shape, self.dense(self.grid).view((-1, self.hidden_size, self.resolution, self.resolution)).shape )   
...
#Ablation study        
# performs softmax instead of sigmoid, required for npp setting
class NPP_SlotAttention_classifier(nn.Module):
    def __init__(self, in_channels, out_channels, mode='softmax' ):
        super(NPP_SlotAttention_classifier, self).__init__()
            
            
        self.mode = mode
        self.dim1 = nn.Linear(in_channels, 256) 
        self.relu = nn.ReLU(inplace=True)
        self.dim2 = nn.Linear(256, out_channels)
        self.softmax = nn.Softmax(dim=1)
    ...
    def forward(self, x, ll_out=False, marg_idx=None, type=None):
        x = self.dim1(x)
        x = self.relu(x)
        x = self.dim2(x)
        if self.mode == 'softmax':
            ...
            col = self.softmax(x[:,0:9])
            shape= self.softmax(x[:,9:13])
            shade= self.softmax(x[:,13:16])
            size= self.softmax(x[:,16:19])
            x=torch.cat([col, shape, shade, size], dim=1)
            return x
        else:
            raise ValueError("activation function not used")
